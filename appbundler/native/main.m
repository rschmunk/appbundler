...
NSString * findJavaDylib (NSString *, bool, bool);
...
int launch(char *commandName, int progargc, char *progargv[]) {
    ...
    // This basically circumvents the security problems introduced with 10.8.4 that JNLP Files
    // must be signed to execute them without CTRL+CLick -> Open
    // See: How to sign (dynamic) JNLP files for OSX 10.8.4 and Gatekeeper
    //  http://stackoverflow.com/questions/16958130/how-to-sign-dynamic-jnlp-files-for-osx-10-8-4-and-gatekeeper
    // There is no solution to properly sign a dynamic jnlp file to date. Both Apple and Oracle
    // have open rdars/tickets on this. The following mechanism encapsulates a JNLP file/template.
    // It makes a temporary copy when executing. This ensures that the JNLP file can be updates
    // from the server at runtime. YES, this may insert additional security threats, but it is
    // still the only way to avoid permission problems. It is highly recommended that the
    // resulting .app container is being signed with a certificate from Apple - otherwise you will
    ...
    if (runtime != nil) {
        ...
        runtimePath = [runtimePath stringByAppendingPathComponent:@"Contents/Home/"];

        for (id dylibRelPath in @[@"jre/lib/jli", @"lib/jli", @"jre/lib", @"lib"]) {
            NSString *candidate = [[runtimePath stringByAppendingPathComponent:dylibRelPath]
                                        stringByAppendingPathComponent:@LIBJLI_DY_LIB];
            BOOL isDir;
                         ...
                         bool exactMatch)
{
    @try
    {
        ...
        if (jvmRequired > 8) {
            args = [NSArray arrayWithObjects: @"-v", [NSString stringWithFormat:@"%i%@", jvmRequired, exactMatch?@"":@"+"], nil];
        } else {
            args = [NSArray arrayWithObjects: @"-v", [NSString stringWithFormat:@"1.%i%@", jvmRequired, exactMatch?@"":@"+"], nil];
        }
        [task setArguments:args];
        ...
        // If JDK is found, outRead will include something like
        ...
        if ( (version >= jvmRequired && !exactMatch) || (version == jvmRequired && exactMatch) ) {
            ...
            // Return location where LIBJLI_DY_LIB is located.
            // Note: The path was shortened between JDK 8 and JDK 9.
            if (version > 8) {
                return outread2;
            }
            else {
...
/**
 *  Extract the Java major version number from a string. We expect the input to look like "1.X",
 *  "1.X.Y_ZZ" or "X.Y.ZZ", and the returned result will be the integral value of X. Any failure
 *  to parse the string will return 0.
 */
int extractMajorVersion (NSString *vstring) {
    ...
    // If the version string uses < 9 layout, remove the leading "1."
    if ([vstring hasPrefix:@"1."]) {
        vstring = [vstring substringFromIndex:2];
    }

    // The next integer token should be the major version, so read everything up to the first
